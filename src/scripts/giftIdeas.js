...
function showGiftIdeas(giftIdeas) {
  ...
  giftIdeas.forEach((giftIdea) => {
    ...
    const { title, description, priceRange } = giftIdea;
    ideas.innerHTML = `
      <h2 class="text-xl font-bold text-red-400">${title}</h2>
      <p class="text-sm">${description}</p>
      <p class="text-base font-bold text-red-400">Price Range: <span class="text-neutral-100 text-sm font-normal">${priceRange}</span></p>
    `;
    main.appendChild(ideas);
  });
}

async function getGiftIdeas(giftData) {
